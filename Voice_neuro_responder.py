...
\r ║ голосовые команды            > #:│{YEL} запрос | история | медицина | комедия | лирика | факт | игра{SRA} ║
                ...
                if rec.AcceptWaveform(stream.read(4000)):
                    ...
                    try:
                        if prompt in ('"поговорим"', '"переводчик"', '"закройся"', '"с свали"', '"свали"'):
                            ...
                            print(LCY + "#: " + words[-1])
                            # Удалите последнее слово
                            full_sentence = full_sentence.rsplit(
                                words[-1], 1)[0]
                            try:
                                trans = translator.translate(
                                    full_sentence, "english", "russian")
